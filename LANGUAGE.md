...
`str`
 ...
 > A null-terminated extended ASCII string, capable of holding up to `32768` characters including the null terminator byte. Implemented as a 16 bit pointer to a memory address in RAM. Takes up two bytes of memory on the stack, stored as a big-endian pointer. Note that non-constant strings are required to specify the desired maximum storage length. This is done using bracket notation as demonstrated in examples below. The exception to this is in functions returning strings which explicitly return a variable or constant. In this case, you are free to declare that your function returns a `str` without using bracket notation to define a maximum storage length. However, if you then use that function in a complex intermediate expression down the line the compiler may not be able to determine a safe size for any necessary intermediate string. If this happens, the compiler will emit an error on the line and you can remediate by either declaring your function return with a maximum string size or by using a temporary string variable as the result of the expression.
...
Unlike Python, MiniPy lets you assign a character to an index in a string. This is because strings are mutable in MiniPy. Note that MiniPy only supports strings up to 32,767 characters in length and has no support for multi-byte unicode values. There is no support for Python's `encode()` and `decode()` functions and there is no support for raw bytestrings because a MiniPy string is effectively a null-terminated bytestring. Unlike Python, each individual character in a string has the data type of `char`, not `str`. That means that performing an operation such as `string[5]` will return a `char` data type, whereas performing an operation such as `string[5:6]` will return a `str` data type. While you can index or slice into strings using 16 bit integers, this is much slower than using 8 bit integers. So, unless you need long string support it is recommended to keep your strings short and your index variables to 8 bit data types. This goes similarly for calculating a string's length. Should you know that the string is short, you should assign the length to an 8 bit integer.
...
`len(str)`
 ...
 > Given a string literal or variable as its only argument, returns the length of the string in characters (not including the null terminator) as a `uint8` or `uint16` depending on what you're assigning to. Note that if you assign to an 8 bit integer but your string is longer than 255 bytes, the length will be wrong. This works with strings up to 32767 characters long as that is the supported length limit of strings in MiniPy
...
MiniPy has extremely limited support for Python's `sys` module. Absolutely no functions are supported but if you `import sys` you will have access to a few constants provided by the compiler that you can use in your code. Note that you can also import specific constants from sys using `from sys import X` or `from sys import *` syntax. The bits of `sys` that are supported are documented below.
